<x-app-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4 py-8 bg-gray-900 mt-12">
        <!-- Header -->
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-4 rounded-lg shadow-lg flex justify-between items-center">
            <h2 class="font-semibold text-2xl flex items-center gap-2">
                <span>✍️</span> Edit Book
            </h2>
            <div class="flex items-center">
                @include('books.partials.delete-book')
            </div>
        </div>

        <!-- Main Content -->
        <div class="mt-8 flex flex-col lg:flex-row gap-8" x-data="{ imageUrl: '/storage/{{ $book->foto }}' }">
            <!-- Form Section -->
            <div class="w-full lg:w-1/2">
                <form enctype="multipart/form-data" method="POST" action="{{ route('books.update', $book) }}" class="bg-gray-800 p-6 rounded-md shadow-md">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="sinopsis" 
                                  ...
                                  placeholder="Write a short synopsis">{{ $book->sinopsis }}</textarea>
                        <x-input-error :messages="$errors->get('sinopsis')" class="mt-2" />
                    </div>

                    <!-- Submit Button -->
                    <div class="flex justify-end mt-6">
                        <x-primary-button class="w-full justify-center bg-indigo-500 hover:bg-indigo-700 transition ease-in-out duration-300 shadow-lg">
                            {{ __('Submit') }}
                        </x-primary-button>
                    </div>
                </form>
